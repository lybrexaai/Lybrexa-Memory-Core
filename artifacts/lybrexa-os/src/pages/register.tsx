...
import { Terminal, Lock, Mail, User, Loader2 } from "lucide-react";
    ...
    <div className="min-h-screen w-full flex items-center justify-center bg-background p-4 font-sans relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-card border border-border rounded-lg p-8 shadow-[0_0_40px_rgba(157,75,255,0.15)] relative z-10">
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-muted-foreground text-sm font-mono mt-2">NEW OPERATOR INITIALIZATION</p>
        </div>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            <label className="text-xs font-mono text-muted-foreground uppercase tracking-wider">Codename (Display Name)</label>
          ...
          >
            {registerMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : "Establish Credentials"}
          </button>
        </form>

        <div className="mt-6 text-center">
          <Link href="/login" className="text-xs text-muted-foreground hover:text-primary transition-colors font-mono">
            [ RETURN TO LOGIN ]
          </Link>
        </div>
      </div>
